...
void Rental(String uid) {
  ChoiceRental(); //Serial.println("등록된 사용자, 우산 대여 진행 중... 대여할 우산을 선택하세요.");
...
void Return(String uid) {
  ChoiceReturn(); //Serial.println("등록된 사용자, 반납 요청 처리 중... 반납할 우산꽂이을 선택하세요.");
  int btn = waitForButtonPress();
  char buffer[256];
  StaticJsonDocument<256> doc;
...
void choice_Rental_Return(JsonDocument& doc){
  ...
  ChoiceGoMain(); //Serial.println("메인으로 돌아갑니다.\n");
}